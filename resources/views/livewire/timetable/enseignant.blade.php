...
new class extends Component {
    ...
    public function with(): array
    {
        ...
        // Classes de l'enseignant (myClassIds le borne ; null = admin voit tout)
        ...
        $selectedId = $this->classId ?: (string) ($classes->first()->id ?? '');

        // Garde : la classe sélectionnée doit être dans le périmètre
        $creneaux = collect();
        if ($selectedId) {
            $inScope = $classes->contains('id', (int) $selectedId);
            if ($inScope) {
                $creneaux = TimetableSlot::where('school_class_id', $selectedId)
        ...
        $selected = $classes->firstWhere('id', (int) $selectedId);

        return compact('classes', 'creneaux', 'selected', 'year');
    }
}; ?>
...
<div>
    ...
    @if ($classes->isEmpty())
        <div class="et-empty">Aucune classe ne vous est assignée pour l'année en cours.</div>
    @else
        <div class="et-picker">
        ...
        @if (! $selected || $creneaux->isEmpty())
            <div class="et-empty">Aucun emploi du temps disponible pour cette classe pour le moment.</div>
        @else
                            ...
                            @forelse ($duJour as $creneau)
                                ...
                                <div class="et-slot" style="background:{{ $c }}14; border-left-color:{{ $c }};">
                                    ...
                                    <div class="et-slot-meta">
                                        @if ($creneau->staff?->user) {{ $creneau->staff->user->name }} @endif
                                        @if ($creneau->room) · Salle {{ $creneau->room }} @endif
                                    </div>
                                </div>
                            @empty
