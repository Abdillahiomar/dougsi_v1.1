...
new class extends Component
{
    ...
    /**
     * Requête principale des élèves ayant des impayés échus.
     *
    ...
    private function rowsQuery()
    {
        ...
        $q = StudentInvoice::withoutGlobalScopes()
            ...
            // Montant payé inférieur au montant dû
            ->whereRaw(
            ...
            // PostgreSQL : comparaison avec la date du jour
            ...
            ->when(
                $this->classId,
                fn ($q) => $q->where('sc.id', $this->classId)
            )
            ...
            ->selectRaw('
                ...
                (
                    CURRENT_DATE
                    - MIN(student_invoices.due_at)::date
                ) AS anciennete
            ')
             ...
             * HAVING reste > 0
             *
             * car "reste" est un alias du SELECT.
             *
             * Il faut répéter l'expression.
             */
            ->havingRaw('
                SUM(
                    student_invoices.amount_due
         ...
         *
         * HAVING anciennete BETWEEN ...
         *
         * On répète donc l'expression.
         */
        $q = match ($this->bucket) {
        ...
        /*
         * PostgreSQL autorise l'utilisation des alias
         * du SELECT dans ORDER BY.
         */
    ...
    public function export()
    {
        ...
        $phones = $this->guardianPhones(
            $rows->pluck('student_id')->all()
        );

        return response()->streamDownload(
            function () use ($rows, $phones) {
            ...
            'impayes-' . today()->format('Y-m-d') . '.csv',
            [
                'Content-Type' =>
    ...
    /**
     * Tuteur principal par élève.
     *
     * Requête séparée afin d'éviter la duplication
     * des lignes agrégées.
     */
        ...
        <div>
            ...
            <div class="page-sub">
                ...
                <strong style="color:var(--accent-red);">
                    {{ number_format($totalReste, 0, ',', ' ') }} DJF
                </strong>
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th style="width:36px;">
                            ...
                            <input
                                type="checkbox"
                                wire:click="toggleAll({{ json_encode($allIds) }})"
                                @checked(
                                    count($selected)
                                    && count($selected) === count($allIds)
                                )
                            >
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    ...
                                    <span
                                        style="opacity:.35;font-size:.8125rem;"
                                    >
                                        Aucun tuteur
                                    </span>
                            ...
                            <td
                                class="num mono"
                                style="color:var(--accent-red);font-size:.875rem;"
                            >
