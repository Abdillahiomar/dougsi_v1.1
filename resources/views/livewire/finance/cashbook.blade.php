    ...
    public function openCloseModal(): void
{
    ...
    public function closeSession(): void
    {
        ...
        if (! $this->closingSessionId) {
            $this->error = 'Aucune session de caisse sélectionnée.';
            return;
        }

        $session = CashSession::withoutGlobalScopes()
            ->where('id', $this->closingSessionId)
        ...
        if (! $session) {
            $this->error = 'La caisse est introuvable ou déjà clôturée.';
            return;
        }

        if ($this->countedCash === '') {
            $this->error = 'Veuillez saisir le montant des espèces comptées.';
            return;
        }
        ...
        if ($counted < 0) {
            $this->error = 'Le montant compté ne peut pas être négatif.';
            return;
        }

        $variance = $counted - $session->expectedCash();

        // Un écart doit obligatoirement être expliqué
        if (
            $variance !== 0 &&
            strlen(trim($this->closingNotes)) < 10
        ) {
            $this->error =
                "Écart de "
                . number_format($variance, 0, ',', ' ')
                . " DJF constaté. "
                . "Une explication d'au moins 10 caractères est obligatoire.";
    ...
    public function confirmVoid(): void
    {
        ...
        if (strlen(trim($this->voidReason)) < 10) {
            $this->error =
                "Le motif d'annulation doit faire au moins 10 caractères.";
        ...
        try {
            ...
            app(PaymentService::class)->void(
                $receipt,
                trim($this->voidReason),
                auth()->id()
            );

            $this->success =
                "Reçu {$receipt->receipt_number} annulé.";
    ...
    public function with(): array
    {
        ...
        $byMethod = PaymentReceipt::withoutGlobalScopes()
            ...
            ->selectRaw(
                'method, COUNT(*) AS n, SUM(amount) AS total'
            )
            ->groupBy('method')
            ->get()
            ->keyBy('method');
        ...
        $liveVariance =
            $this->countedCash !== ''
                ? ((int) $this->countedCash) - $expectedCash
                : null;
        ...
        $voidingReceipt =
            $this->voidingId
                ? PaymentReceipt::find($this->voidingId)
                : null;

        /*
         * Session utilisée par le modal.
         *
         * On la récupère grâce à closingSessionId
         * afin de ne pas dépendre uniquement de $session.
         */
        $closingSession = null;

        if ($this->closingSessionId) {
            $closingSession = CashSession::withoutGlobalScopes()
                ->where('id', $this->closingSessionId)
                ->where('school_id', $schoolId)
                ->where('user_id', auth()->id())
                ->where('status', 'open')
...
<div>
        ...
        <div class="fin-alert ok">
            ...
            <svg fill="none"
                ...
                <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0a9 9 0 0118 0z"/>
            </svg>
    ...
    <div class="cashbook-bar">
        ...
        @if ($session)
            ...
            <div class="m-box session">
                ...
                <div class="mono" style="margin-top:2px;">
                    {{ number_format($session->opening_float, 0, ',', ' ') }}
                    DJF
                </div>
            ...
            <div class="m-box session">
                ...
                <div
                    class="mono"
                    style="margin-top:2px;color:#166534;"
                >
        ...
        @foreach (\App\Models\PaymentReceipt::METHODS as $key => $label)
            ...
            <div class="m-box">
                ...
                <div class="m-val">
                    {{ number_format((int)($row->total ?? 0), 0, ',', ' ') }}
                </div>
        ...
        @can('finance.close')
            ...
            >
                ...
                >
                    ...
                    <span wire:loading.remove wire:target="openCloseModal">
                        Clôturer la caisse
                    </span>

                    <span wire:loading wire:target="openCloseModal">
                        Ouverture...
                    </span>
    ...
    <div class="filters">
        ...
        <label
            style="display:flex;align-items:center;gap:.4rem;font-size:.8125rem;cursor:pointer;padding-bottom:.45rem;"
        >

            <input
                type="checkbox"
                wire:model.live="showVoided"
            >
                ...
                <tbody>
                    ...
                    @forelse ($receipts as $r)
                        ...
                        <tr class="{{ $r->isVoided() ? 'row-voided' : '' }}">
                            ...
                            <td
                                style="font-size:.8125rem;opacity:.65;"
                            >
                                {{ $r->receivedBy?->name }}
                            </td>
                                ...
                                >
                                    <svg fill="none"
                                        ...
                                        <path
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 002 2v4m8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
                                    </svg>
                                </a>
                        ...
                        @if ($r->isVoided() && $r->void_reason)
                            ...
                            <tr>
                                ...
                                <td
                                    colspan="8"
                                    style="padding:.35rem .85rem .7rem;border-bottom:1px solid var(--line);"
                                >
                    ...
                    <tbody>
                        ...
                        @foreach ($recentSessions as $s)
                            ...
                            <tr>
                                ...
                                <td
                                    style="font-size:.8125rem;opacity:.6;"
                                >
                                    {{ $s->opened_at->format('d/m H:i') }}
                                    →
                                    {{ $s->closed_at?->format('d/m H:i') }}
                                </td>

                                <td class="num mono">
                                    {{ number_format((int)$s->expected_cash, 0, ',', ' ') }}
                                </td>

                                <td class="num mono">
                                    {{ number_format((int)$s->counted_cash, 0, ',', ' ') }}
                                </td>

                                <td
                                    class="num mono"
                                    style="color:{{ $s->variance == 0
                                        ? '#166534'
                                        : ($s->variance < 0
                                            ? 'var(--accent-red)'
                                            : '#8A6010') }};"
                                >
                                    {{ $s->variance > 0 ? '+' : '' }}
                                    {{ number_format((int)$s->variance, 0, ',', ' ') }}
                                </td>

                                <td
                                    style="font-size:.75rem;opacity:.55;"
                                >
                                    {{ $s->notes }}
                                </td>
    ...
    @if ($closing && $closingSession)

        <div
            class="modal-back"
            wire:click.self="cancelCloseModal"
        >
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    ...
                    >
                        ...
                        <span class="mono">
                            {{ number_format(
                                $expectedCash - $closingSession->opening_float,
                                0,
                                ',',
                                ' '
                            ) }}
                    ...
                    >
                        ...
                        <input
                            wire:model.live.debounce.400ms="countedCash"
                            type="number"
                            min="0"
                            class="fin-input"
                            style="
                    ...
                    <div class="filter-field">
                        ...
                        <textarea
                            wire:model.live="closingNotes"
                            rows="2"
                            class="fin-input"
                            placeholder="Explication de l'écart, incident…"
                        ></textarea>
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    ...
                    <div class="fin-alert warn">
                        ...
                        <div>
                            ...
                            {{ number_format(
                                $voidingReceipt->amount,
                                0,
                                ',',
                                ' '
                            ) }}
                            DJF seront retirés du solde de

                            {{ $voidingReceipt->student?->fullName() }}.

                            Le reçu reste conservé et visible,
                            marqué comme annulé.
                    ...
                    <div class="filter-field">
                        ...
                        <textarea
                            wire:model.live="voidReason"
                            rows="3"
                            class="fin-input"
                            placeholder="Erreur de saisie du montant, chèque sans provision…"
                            autofocus
                        ></textarea>
                ...
                <div class="modal-foot">
                    ...
                    <button
                        ...
                        "
                        @disabled(strlen(trim($voidReason)) < 10)
                    >
